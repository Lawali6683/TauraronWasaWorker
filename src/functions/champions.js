...
async function handleRequest(request) {
    ...
    try {
        ...
        const CL_MATCHES_URL = `https://api.football-data.org/v4/competitions/CL/matches${stage ? `?stage=${stage}` : ''}`;
        const DED_TABLE_URL = 'https://api.football-data.org/v4/competitions/DED/standings';
        const CL_SCORERS_URL = 'https://api.football-data.org/v4/competitions/CL/scorers?limit=10';
        
        const [matchesResponse, tableResponse, scorersResponse] = await Promise.all([
            fetch(CL_MATCHES_URL, { headers: { 'X-Auth-Token': FOOTBALL_API_TOKEN } }),
            fetch(DED_TABLE_URL, { headers: { 'X-Auth-Token': FOOTBALL_API_TOKEN } }),
            fetch(CL_SCORERS_URL, { headers: { 'X-Auth-Token': FOOTBALL_API_TOKEN } })
        ]);
        ...
        return new Response(JSON.stringify({ error: true, message: 'Server error while fetching all data.', details: e.message }), { status: 500, headers: { 'Content-Type': 'application/json' } });
    }
}